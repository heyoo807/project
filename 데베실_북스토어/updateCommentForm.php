		...
		$comment_id = $_GET['a'];
		 $mysqli = new mysqli("localhost", "heyoo", "2021103996", "heyoo");
		      if (mysqli_connect_errno()) {
		        ...
		        $sql = "SELECT * FROM P_COMMENT WHERE comment_id = '".$comment_id."'";
		        $res = mysqli_query($mysqli, $sql);
		        	...
		        	while ($newArray = mysqli_fetch_array($res, MYSQLI_ASSOC)){
		        		...
		        		if($user_id != $member_id){
										echo "<script>alert('본인 게시물만 수정 가능합니다');</script>";
										echo "<script>location.replace('./productinfo.php?a=$product_id');</script>";
										
									}
							echo "<div class='comment-section'>";
					    echo "<form action = './updateCommentResp.php' method = 'get'>";
					    echo "<h2>"."Comments"."</h2>";
					    ...
					    echo "</div>";
						  ...
						  echo "<tbody>";
							  ...
							  echo "<td>".$comment_content."</td>";
								echo "<td>";
								echo "<a href='./updateCommentForm.php' class='uda'>"."update"."</a>";
			          echo "</td>";
			          echo "<td>";
